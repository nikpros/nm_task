          ...
          <v-row>
            <v-col cols="12">
              <v-text-field
                ...
                :error-messages="
                  !$v.title.required && $v.title.$dirty
                    ? 'This is a required field'
                    : !$v.title.minLength
                    ? 'Title must be container 4 symbols'
                    : ''
                "
                v-model="title"
              ></v-text-field>
            ...
            <v-col cols="6">
              <v-text-field
                ...
                v-model="time_str"
                :error-messages="
                  $v.time_str.$dirty && !$v.time_str.mustBeMoreThenNow
            ...
            <v-col cols="6">
              <v-text-field label="Finish time" v-model="time_fsh" type="time">
              </v-text-field>
            </v-col>
            <v-col cols="12">
              <v-autocomplete
            ...
            <v-col cols="6">
              <v-select
                ...
                :error-messages="
                  !$v.role.facilitator.required && $v.role.facilitator.$dirty
                    ? 'Facilitator is required'
                    : role.secretary === role.facilitator &&
                      $v.role.facilitator.required
                    ? 'Invalid match'
                    : ''
                "
            ...
            <v-col cols="6">
              <v-select
                ...
                :disabled="role.participants.length < 2"
                :error-messages="
                  !$v.role.secretary.required && $v.role.secretary.$dirty
                    ? 'Secretary is required'
                    : role.secretary === role.facilitator &&
                      $v.role.secretary.required
                    ? 'Invalid match'
                    : ''
                "
...
function mustBeMoreThenNow(value) {
  ...
  if (
    now.getHours() > start_hours ||
    (now.getHours() == start_hours && now.getMinutes() >= start_minutes)
  )
    return false;
  else return true;
}
      ...
      let noAll = all.filter(user => {
        ...
        if ("membership" in user && user.membership.length != 0) {
          for (let i = 0; i < user.membership.length; i++) {
            let needMeeting = this.$store.getters.allMeetings.find(
                value => value.title === user.membership[i].title
              ),
              time_str = needMeeting.time_str,
              time_fsh = needMeeting.time_fsh;

            if (this.time_str > time_fsh || this.time_fsh < time_str)
